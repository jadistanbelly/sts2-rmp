...
public class ConfigManager
{
	...
	public void Save()
	{
		...
		{
			Log.Warn("[RMP] Failed to save config: " + ex.Message, 2);
		}
	}
	...
	private void LoadOrCreateConfig()
	{
		...
		if (File.Exists(_configPath))
		{
			...
			{
				Log.Warn("[RMP] Failed to parse config: " + ex.Message, 2);
	...
	private void MigrateLegacyJsonConfig(string jsonPath)
	{
		try
		{
			...
			Log.Info("[RMP] Migrated config.json to config.ini", 2);
		}
		catch (Exception ex)
		{
			Log.Warn("[RMP] Failed to migrate legacy config: " + ex.Message, 2);
		}
	}
	...
	private static string ResolveModDirectory()
	{
		...
		string text2 = Path.Combine(AppContext.BaseDirectory, "mods", "RemoveMultiplayerPlayerLimit");
		if (Directory.Exists(text2))
		{
			return text2;
		}
		return Path.Combine(Environment.GetFolderPath(Environment.SpecialFolder.ApplicationData), "StS2Mods", "RemoveMultiplayerPlayerLimit");
	}
