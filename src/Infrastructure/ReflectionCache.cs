...
public class ReflectionCache
{
	...
	public MethodInfo? GetMethod(string typeName, string methodName, Type[]? paramTypes = null)
	{
		...
		if (!_methods.TryGetValue(key, out MethodInfo value))
		{
			...
			value = ((paramTypes != null) ? type?.GetMethod(methodName, BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic, null, paramTypes, null) : type?.GetMethod(methodName, BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic));
			_methods[key] = value;
		}
		return value;
	}
